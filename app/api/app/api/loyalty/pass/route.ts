...
/**
 * Tiers MUST match your iOS tiers (names + minPoints).
 * From your Xcode:
 * Starter: 0
 * Community: 100
 * Friend: 500
 * Family: 1000
 * The Inner Circle: 4000
 */
...
function normalizeE164(phone: string): string {
  const p = phone.trim();
  if (!p.startsWith("+")) throw new Error("phone must be E.164 (must start with +)");
  return p;
}
...
async function squareFetch<T>(endpoint: string, method: "GET" | "POST", body?: any): Promise<T> {
  ...
  if (!res.ok) {
    const msg = json?.errors?.[0]?.detail || json?.errors?.[0]?.code || json?.message || text || `Square error ${res.status}`;
    throw new Error(msg);
  }
...
async function getSquareCustomerByPhone(phoneE164: string): Promise<{ id: string; name: string }> {
  // Square Customers: /v2/customers/search
  const payload = {
    query: {
      filter: {
        phoneNumber: { exact: phoneE164 }
      }
    },
    limit: 1
  };
...
async function getLoyaltyPointsByPhone(phoneE164: string): Promise<number> {
  // Square Loyalty: /v2/loyalty/accounts/search
  // Search by mapping phone number
  const payload = {
    query: {
...
function readAsset(rel: string): Buffer {
  // assets committed here:
  // app/api/loyalty/pass/assets/{WWDR.pem, icon.png}
...
async function extractPemFromP12(tmpDir: string) {
  ...
  // key (private) - keep encrypted using same passphrase for simplicity
  await execFileAsync("openssl", [
    "pkcs12",
    "-in", p12Path,
    "-nocerts",
...
export async function GET(req: Request) {
  try {
    ...
    // 2) Compute tier (matches iOS tiers)
    const tier = tierForPoints(points);

    // 3) Build pass.json (Member name + Tier)
    const passTypeId = mustEnv("PASS_TYPE_ID");     // pass.com.casamarana.loyalty
    const teamId = mustEnv("APPLE_TEAM_ID");        // Y32NA77D5W

    // Unique serial per customer; stable without storing DB
    ...
    const passJson = {
      ...
      storeCard: {
        primaryFields: [
          { key: "tier", label: "Tier", value: tier.name }
        ],
        secondaryFields: [
    ...
    // 4) Required files inside pkpass
